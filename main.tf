...
resource "aws_network_interface" "Monitor-Server" {
  subnet_id       = aws_subnet.JenkinsSubnet.id
...
resource "aws_eip" "Monitor-Server" {
  domain                    = "vpc"
}

# Associate EIP to EC2 instances ENI

resource "aws_eip_association" "eip_assoc_to_Monitor-Server" {
  instance_id   = aws_instance.Monitor-Server.id
  allocation_id = aws_eip.Monitor-Server.id
}
...
resource "aws_instance" "Monitor-Server" {
  ...
  user_data         = file("./scripts/monitor.sh")
  root_block_device {
    volume_size = 15
    volume_type = "gp3"
    encrypted   = true
  ...
  network_interface {
    device_index         = 0
    network_interface_id = aws_network_interface.Monitor-Server.id
  }
}

#Output
output "Monitor-Server" {
  value = "ssh -i ~/${var.key_pair}.pem ubuntu@${aws_eip.Monitor-Server.public_ip}"
}
